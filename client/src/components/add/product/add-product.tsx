...
const AddProduct = ({ onProductAdded, onClose }: Props) => {
  ...
  const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const upload = async () => {
      try {
        ...
        setForm((current) => ({ ...current, urlImage: secureUrl }));
        setPreviewImage(secureUrl);
      } catch (error) {
        setForm((current) => ({ ...current, urlImage: "" }));
        setPreviewImage("");
        showToast(
          "Error",
  ...
  const handleSubmit = async () => {
    ...
    if (imageMode === "upload" && isUploadingImage) {
      showToast("Error", "Vui lòng đợi ảnh upload xong trước khi tạo sản phẩm");
      return;
    }
          ...
          <div className="grid grid-cols-3 gap-6 px-6 py-6">
            ...
            <div className="col-span-2 space-y-4">
              ...
              <div className="flex flex-col gap-1">
                <label
                  htmlFor="name"
                  className="text-blue-700 font-medium after:content-['*'] after:text-red-700"
                >
                  Tên sản phẩm
                </label>
                <input
            ...
            <div className="col-span-1 space-y-4">
              ...
              <div className="flex flex-col gap-2">
                <label className="text-blue-700 font-medium">Gắn Tag</label>
