...
const AdminSidebar = ({
  ...
  useEffect(() => {
    const path = window.location.pathname.split("/admin/");
    setUrlParams(path[path.length - 1]);
  }, []);

  return (
    <div
      className={`fixed md:sticky top-0 start-0 w-full sm:w-60 h-screen bg-[#191919] px-4 shadow flex flex-col
        ${isOpenSidebar ? "block" : "hidden"}`}
    >
      <div className="flex justify-between items-center py-3">
        <img src={logo} alt="" className="w-40" />
        <button
          className="md:hidden text-gray-300 hover:text-white w-8 h-8"
                    ...
                    <div>
                      {item.children ? (
                        ...
                        <div className="overflow-hidden">
                          ...
                          <div
                            className="w-full flex justify-between px-2 py-2 bg-[#191919] relative z-1 
                          ...
                          >
                            ...
                            <div
                              onClick={() => toggleChildren(actIdx, itemIdx)}
                            >
                              <button>
                          ...
                          {item.children.isActive && (
                            <div className="activate-children relative z-0 ps-3">
                              {item.children.items &&
                                item.children.items.map((child, idx) => (
                                  <a
                                    key={idx}
                                    href={child.url}
